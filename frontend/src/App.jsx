...
function App({ actor }) {
  ...
  const currentMember = members.find(member => member.user_id === actor.id || member.access_user_id === actor.id)
...
export function Summary({ members = [], group, expenses, balances, settlements, onAdd, onSetup, payments = [], onPay, onPaymentsChanged, onBusy, busy }) {
  ...
  return <>
    ...
    <ShareSummary key={group.id} group={group} expenses={expenses} balances={balances} settlements={settlements} busy={busy} />
